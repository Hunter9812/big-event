...
public class JwtUtils {
    ...
    public String createJwt(UserDetails user, String username, int userId) {
        if(this.frequencyCheck(userId)) {
            //HMAC-SHA256算法是一种在信息安全领域广泛使用的哈希算法，它可以将任意长度的消息变换成具有固定长度输出的消息摘要。
            Algorithm algorithm = Algorithm.HMAC256(key);
    ...
    public DecodedJWT resolveJwt(String headerToken){
        ...
        JWTVerifier jwtVerifier = JWT.require(algorithm).build();
        try {
            DecodedJWT verify = jwtVerifier.verify(token); //如果token被用户篡改过，会抛出验证异常。
            if(this.isInvalidToken(verify.getId())) return null;
            Map<String, Claim> claims = verify.getClaims(); //获取令牌中的声明信息
            return new Date().after(claims.get("exp").asDate()) ? null : verify;
        } catch (JWTVerificationException e) {
            return null; //验证失败，返回为空
        }
    }
    ...
    private boolean deleteToken(String uuid, Date time){
        if(this.isInvalidToken(uuid))
            return false;
        Date now = new Date();
        long expire = Math.max(time.getTime() - now.getTime(), 0); //防止出现负值
